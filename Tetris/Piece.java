...
public class Piece {
	...
	*/
	...
	private void createSkirt(TPoint[] points) {
		int maxX = -1;
		int maxHeight = -1;
		...
		for (int i = 0; i < points.length; i++) {
			int curMinY = points[i].y;
			int curInd = points[i].x;
			if(curMinY < skirt[curInd]){
				skirt[curInd] = curMinY;
			}
		}
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		for(int i = 0; i < this.body.length; i++){
			TPoint cur = body[i];
			if(!Arrays.asList(other.body).contains(cur))
				return false;
		}
